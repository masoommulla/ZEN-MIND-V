...
import { Lock, Mail, Eye, EyeOff, ArrowLeft, Sparkles, Stethoscope, KeyRound } from 'lucide-react';
...
interface TherapistLoginProps {
  onLoginSuccess: (therapist: any, token: string) => void;
  onBack: () => void;
}

export function TherapistLogin({ onLoginSuccess, onBack }: TherapistLoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/api/therapist-auth/login`, {
      ...
      if (response.data.success) {
        ...
        // Store therapist data and token
        localStorage.setItem('therapistToken', response.data.data.token);
        localStorage.setItem('currentTherapist', JSON.stringify(response.data.data.therapist));
        
        onLoginSuccess(response.data.data.therapist, response.data.data.token);
      }
    } catch (error: any) {
  ...
  const handleForgotPasswordEmail = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_URL}/api/therapist-auth/forgot-password`, { email: forgotEmail });
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl overflow-hidden">
          ...
          <div className="p-8 text-center border-b border-white/10">
            ...
            <p className="text-gray-300">
              {forgotPasswordMode ? 'Reset your password' : 'Access your professional dashboard'}
            </p>
          </div>
          ...
          <div className="p-8">
            {!forgotPasswordMode ? (
              ...
              <form onSubmit={handleForgotPasswordEmail} className="space-y-6">
